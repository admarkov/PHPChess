<?php
    ...
    function coordinatesToChessNotation($y, $x)
    {
        if (!State::checkCoordinates($y, $x))
        {
            return null;
        }
        return chr(ord('1') + $y).chr(ord('A') + $x);
    }

    // Получает координату клетки в шахматной нотации (E2)
    // и возвращает пару координат (y, x) в 0-индексации
    // или (-1, -1), если координата некорректна
    function resolveChessNotation($z) {
        $y = ord($z[1]) - ord('1');
        $x = ord($z[0] - ord('A'));
        return array($y, $x);
    }
